...
describe('InventarioService', () => {
  ...
  describe('getAll', () => {
    it('should return paginated inventory items when no filters', async () => {
      const mockItems = [
    ...
    it('should filter by tipo when provided', async () => {
      const mockItems = [{ id: 1, nombre: 'Rosas', tipo: 'flores', stock: 50, stock_min: 10 }];
      query
    ...
    it('should filter by low stock when stock_bajo is true', async () => {
      const mockItems = [
      ...
      expect(query).toHaveBeenCalledWith(
        expect.stringContaining('stock <= stock_min'),
        expect.any(Array)
      );
      expect(result.data).toEqual(mockItems);
    });
    ...
    it('should filter by tipo and low stock together', async () => {
      const mockItems = [{ id: 1, nombre: 'Rosas', tipo: 'flores', stock: 5, stock_min: 10 }];
      query
        .mockResolvedValueOnce({ rows: [{ total: '1' }] })
        .mockResolvedValueOnce({ rows: mockItems });
  ...
  describe('getById', () => {
    it('should return inventory item by id', async () => {
      ...
      expect(query).toHaveBeenCalledWith('SELECT * FROM inventario WHERE id = $1', [1]);
  ...
  describe('create', () => {
    it('should create inventory item with valid data', async () => {
      const newItem = { nombre: 'Rosas', tipo: 'flores', stock: 50, stock_min: 10, unidad: 'docena', costo: 25.50 };
    ...
    it('should throw error when nombre is empty', async () => {
      await expect(inventarioService.create({ nombre: '', tipo: 'flores', stock: 50, costo: 25.50 }))
        .rejects.toThrow('El nombre es requerido');
    });

    it('should throw error when tipo is invalid', async () => {
      await expect(inventarioService.create({ nombre: 'Rosas', tipo: 'invalid', stock: 50, costo: 25.50 }))
        .rejects.toThrow('El tipo debe ser: flores, materiales o accesorios');
    });

    it('should throw error when stock is negative', async () => {
      await expect(inventarioService.create({ nombre: 'Rosas', tipo: 'flores', stock: -5, costo: 25.50 }))
        .rejects.toThrow('El stock debe ser mayor o igual a 0');
    });

    it('should throw error when costo is negative', async () => {
      await expect(inventarioService.create({ nombre: 'Rosas', tipo: 'flores', stock: 50, costo: -10 }))
        .rejects.toThrow('El costo debe ser mayor o igual a 0');
    });
  });

  describe('update', () => {
    it('should update inventory item with valid data', async () => {
      const updateData = { nombre: 'Rosas Rojas', tipo: 'flores', stock: 60, stock_min: 15, unidad: 'docena', costo: 30.00 };
    ...
    it('should throw error when nombre is empty string', async () => {
      await expect(inventarioService.update(1, { nombre: '  ', tipo: 'flores', stock: 50, costo: 25.50 }))
        .rejects.toThrow('El nombre no puede estar vacío');
    });

    it('should throw error when tipo is invalid', async () => {
      await expect(inventarioService.update(1, { nombre: 'Rosas', tipo: 'invalid', stock: 50, costo: 25.50 }))
        .rejects.toThrow('El tipo debe ser: flores, materiales o accesorios');
    });
  });
  ...
  describe('delete', () => {
    it('should delete inventory item by id', async () => {
      ...
      expect(query).toHaveBeenCalledWith('DELETE FROM inventario WHERE id = $1', [1]);
    });
  });
});
